...
def read_all():
    ...
    with the complete list of movie, sorted by movie release date
    ...
    #movie = Movie.query.order_by(Movie.id).slice(2,10)
...
def read_offset_limit(offset,limit):
    """
    This function responds to a request for /api/movie/{offset}/{limit}
    with the limited list of movie, sorted by movie release date
    :return:                json list of all movie for all director
    """
    # Query the database for all the movies
    #movie = Movie.query.order_by(db.desc(Movie.id)).all()
...
def update_movie(movie_id, movie):
    ...
    if update_movie is not None:
        ...
        movie = Movie.query.filter(Movie.uid == update.uid).one_or_none()
        # Was uid already exist?
        if movie is not None:
            abort(404, f"Uid is already used, uid: {update.uid}")
